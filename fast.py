...
os.system("title Hyper Aim Close Range")
...
class Aimbot:
    ...
    def __init__(
        self, box_constant=325, collect_data=False, mouse_delay=0.0000001, debug=False
    ):
...
def on_release(key):
    ...
    global aimbotEnabled
    try:
        if key == keyboard.KeyCode.from_char("3") and started:
            Aimbot.update_status_aimbot()
            if aimbotEnabled:
                overlay.show()
            else:
